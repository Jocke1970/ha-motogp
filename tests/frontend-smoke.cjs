/* Run with: node tests/frontend-smoke.cjs */
'use strict';
...
const session = (date, category, name, status = '') => ({ id: `${category}-${name}`, date, category, name, status });
const sessions = [
...
let entities = {
  'sensor.motogp_next_race': state('Test GP', { sessions_all: sessions, schedule_categories: ['MotoGP', 'Moto2', 'Moto3'] }),
  ...
  'sensor.motogp_rider_positions': state('1 rider', { category: 'Moto3', riders: [{ position: 1, surname: 'TEST', team: 'Honda', last_lap_time: '1:40.000', number: 5 }] }),
...
push({ 'sensor.motogp_session_status': state('In Progress', { category: 'Moto3', session_status_id: 'S' }) });
...
const liveCount = (card._view.innerHTML.match(/class="tile live-tile"/g) || []).length;
assert.equal(liveCount, 1, 'Category-aware LIVE highlights only one session');
click({ timing: '' });
...
assert.match(card._view.innerHTML, /data-timing aria-expanded="false"/, 'Live updates do not force reopen');
push({
  ...
  'sensor.motogp_rider_positions': state('1 rider', { category: 'MotoGP', riders: [{ position: 1, surname: 'NEW', number: 7 }] })
});
assert.match(card._view.innerHTML, /data-timing aria-expanded="true"/, 'New active session reopens');
...
push({
  'sensor.motogp_current_session': state('FP2', { category: 'Moto3', event: 'Test GP' }),
  'sensor.motogp_session_status': state('Not Started', { category: 'Moto3', session_status_id: 'N' })
});
assert.match(card._view.innerHTML, /Senaste timing: MotoGP · FP1/, 'Last snapshot stays after feed switch');
now = Date.parse('2026-09-19T00:01:00+02:00');
card._render();
assert.equal(card._manualDay, null, 'Manual day resets at midnight');
console.log('PASS: registration, defaults, local clicks, category match, live transitions, snapshot, midnight day reset');
