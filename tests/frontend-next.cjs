...
const registry = new Map(), mobile = class OriginalMobile {};
registry.set('ha-motogp-card', mobile);
class Element {
  attachShadow() {
    this.view = {innerHTML:''};
...
assert.equal(registry.get('ha-motogp-card'),mobile);
assert.equal(Card.buildInfo.version,'0.2.0-dev.2');
vm.runInNewContext(code,ctx);
assert.equal(registry.get('ha-motogp-next-card'),Card,'Duplicate resource does not replace element');
...
const opened=(day,v)=>assert.match(html(),new RegExp(`data-day="${day}" aria-expanded="${v}"`));
push({});
assert.match(html(),/GRAND PRIX OF AUSTRIA/);
assert.match(html(),/18–20 sep/,'Readable header dates');
...
// Confirmed final session opens tomorrow beside today's gray sessions.
...
click({day:'2026-09-19'});opened('2026-09-19',false);opened('2026-09-18',true);
click({day:'2026-09-19'});opened('2026-09-19',true);
// Filtering does not invent a date without any sessions in that class.
...
// If API leaves completed sessions NOT-STARTED, never label them FINISHED.
...
// Real active feed suspends next-day preview; timing still auto-opens.
...
assert.match(html(),/data-timing aria-expanded="true"/);assert.match(html(),/RIDER ONE/);
...
click({timing:''});assert.match(html(),/RIDER ONE/);
push({'switch.motogp_no_spoiler':state('on')});
assert.doesNotMatch(html(),/RIDER ONE/);assert.equal(card._snapshot,null);
...
console.log('PASS: dual-day preview, independent toggles, filter, explicit/presumed completion, no-weather, timing, spoiler, midnight');
