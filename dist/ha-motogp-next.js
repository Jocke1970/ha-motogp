/* MotoGP Next SINGLE Lovelace resource | build next-one-20260920-03 | read only. */
...
(() => {
  ...
  class MotoGPNext extends HTMLElement {
    ...
    _render() {
      ...
      this.shadowRoot.getElementById('app').innerHTML=`<div class="root"><section class="panel header"><div class="eyebrow">🏁 MOTOGP NEXT · 0.3.0-dev.3</div><h2>${esc(event)}</h2><div class="sub">${meta.map(esc).join(' · ')||'Väntar på evenemangsdata'}</div></section>`+
        this._schedule(race,now)+`<div class="root">${this._weather()}${this._timing()}</div>`+
        `<div class="foot">ha-motogp-next.js · 0.3.0-dev.3 · samlad testversion</div></div>`;
    }
